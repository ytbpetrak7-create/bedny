  ...
  function doPost(e) {
    ...
    case "getSteamInventoryAll":
      ...
      break;
    case "getSteamInventoryDirect":
      result = getSteamInventoryDirect(ss, params.username);
      break;
  ...
  function getSteamInventoryDirect(ss, username) {
    if (!username) return JSON.stringify([]);
    var usersSheet = getSheet(ss, "Users");
    var data = usersSheet.getDataRange().getValues();
    var steamId = null;
    ...
    try {
      var url = "https://steamcommunity.com/inventory/" + steamId + "/730/2?l=english&count=500";
      ...
      var code = response.getResponseCode();
      if (code !== 200) return JSON.stringify({ error: "HTTP_" + code, steamId: steamId });
